const uiService = {
    ...
    // UPDATED: Now accepts the PagedResult object
    // UPDATED: Now accepts the PagedResult object and calculates sequential IDs
    renderEmployeeTable: (pagedResult) => {
        ...
        // Update the count label based on API data
        $('#showing-count-label').text(`Showing ${employees.length} of ${pagedResult.totalCount} employees`);

        if (employees.length === 0) {
            $('#empty-state').removeClass('d-none');
            tbody.closest('table').addClass('d-none');
            $('#pagination-container').empty(); // Clear pagination
            return;
        }
        ...
        // NEW: We added 'index' to the forEach loop parameters
        employees.forEach((emp, index) => {
            const initial = uiService.getInitials(emp.firstName, emp.lastName);
            const statusBadge = emp.status === 'Active' ? 'success' : 'danger';
            const deptColor = uiService.getDeptColor(emp.department);
            
            // NEW: Calculate a continuous sequential row number based on the current page
            // (e.g., Page 1 shows 1-10, Page 2 shows 11-20)
            const displayId = (pagedResult.page - 1) * CONFIG.PAGE_SIZE + (index + 1);
            ...
            const tr = `
                <tr>
                    ...
                    <td class="align-middle action-buttons">
                        ...
                        <button class="btn btn-sm btn-outline-secondary me-1 btn-edit admin-only" data-id="${emp.id}"><i class="bi bi-pencil"></i></button>
                        <button class="btn btn-sm btn-outline-danger btn-delete admin-only" data-id="${emp.id}"><i class="bi bi-trash"></i></button>
                    </td>
                </tr>`;
            tbody.append(tr);
        });
        ...
        uiService.renderPagination(pagedResult);
        
        // Enforce Role UI immediately after rendering rows
        uiService.applyRoleUI();
    },

    // NEW: Render server-side pagination buttons
    renderPagination: (pagedResult) => {
        const container = $('#pagination-container');
        container.empty();

        if (pagedResult.totalPages <= 1) return; // Don't show pagination if only 1 page

        // Previous Button
        container.append(`
            <li class="page-item ${!pagedResult.hasPrevPage ? 'disabled' : ''}">
                <button class="page-link page-btn" data-page="${pagedResult.page - 1}">Previous</button>
            </li>
        `);
        ...
        container.append(`
            <li class="page-item ${!pagedResult.hasNextPage ? 'disabled' : ''}">
                <button class="page-link page-btn" data-page="${pagedResult.page + 1}">Next</button>
            </li>
        `);
    },

    // NEW: Hide elements that Viewers shouldn't see
    applyRoleUI: () => {
        if (!authService.isAdmin()) {
            $('.admin-only').hide(); // Hides the edit/delete buttons on table rows
        } else {
            $('.admin-only').show();
        }
    },
    ...
    populateDepartmentDropdown: (departments) => {
        ...
        // Only keep 'All' option, remove others
